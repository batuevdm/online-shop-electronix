...
class OrdersController extends Controller
{
    ...
    public function dashboard_view()
    {
        ...
        if ($count > 0) {
            ...
            $orders = $this->models->order->getAll(($page - 1) * $productsOnPage, $productsOnPage, 5);
            if ($orders) {
                for ($i = 0; $i < count($orders); $i++) {
                    ...
                    $orders[$i]['sum'] = $sum;
                    $orders[$i]['status'] = Config::get('order.status')[$orders[$i]['status']];
                }
    ...
    public function dashboard_order()
    {
        ...
        if (isset($_POST['status'])) {
            ...
            $res = $this->models->order->editStatus($id, $status);
            if ($res) {
                Session::setMessage('Статус изменен', 'success');
            } else {
                Session::setMessage('Ошибка', 'error');
            }
            App::getRouter()->redirect('/dashboard/orders/order/'.$id);
        }
        ...
        if ($order) {
            ...
            $this->data['address'] = $this->models->user->getAddress($order['address'], $order['user'])['address'];
        } else {
            App::getRouter()->redirect('/dashboard/orders/view');
            exit();
        }
